...
import FireResponsePlan from './FireResponsePlan';
import { 
  Flame, 
  Droplets, 
  Wind, 
...
const SmartFireSensePage: React.FC = () => {
  ...
  const [activeSubTab, setActiveSubTab] = useState<'dashboard' | 'architecture' | 'ai-plan' | 'codehub' | 'api'>('dashboard');
  ...
  // Generate historical wildfire risk and vegetation health trends (fetched as if from analysis engine)
  const getHistoricalDataForArea = (region: typeof selectedRegionRisk) => {
    if (!region) {
      // Default baseline data for general forest reserve
      const baseMonths = [
        { name: language === 'fa' ? 'اردیبهشت' : 'May', risk: 25, ndvi: 78, moisture: 65 },
        { name: language === 'fa' ? 'خرداد' : 'Jun', risk: 38, ndvi: 72, moisture: 54 },
        { name: language === 'fa' ? 'تیر' : 'Jul', risk: 62, ndvi: 55, moisture: 35 },
        { name: language === 'fa' ? 'مرداد' : 'Aug', risk: 78, ndvi: 42, moisture: 22 },
        { name: language === 'fa' ? 'شهریور' : 'Sep', risk: 68, ndvi: 48, moisture: 28 },
        { name: language === 'fa' ? 'مهر' : 'Oct', risk: 35, ndvi: 64, moisture: 45 }
      ];
      return baseMonths.map(m => ({
        month: m.name,
        'ریسک آتش‌سوزی (%)': m.risk,
        'Wildfire Risk (%)': m.risk,
        'شاخص سلامت گیاهی (NDVI %)': m.ndvi,
        'Vegetation Health (NDVI %)': m.ndvi,
        'رطوبت خاک (%)': m.moisture,
        'Soil Moisture (%)': m.moisture
      }));
    }

    const lat = region.center[0];
    const lng = region.center[1];
    const baseRisk = region.riskPercent;

    const months = [
      { name: language === 'fa' ? 'اردیبهشت' : 'May', tempOffset: -12, humOffset: 20, ndviOffset: 12 },
      { name: language === 'fa' ? 'خرداد' : 'Jun', tempOffset: -5, humOffset: 10, ndviOffset: 5 },
      { name: language === 'fa' ? 'تیر' : 'Jul', tempOffset: 4, humOffset: -5, ndviOffset: -4 },
      { name: language === 'fa' ? 'مرداد' : 'Aug', tempOffset: 10, humOffset: -12, ndviOffset: -10 },
      { name: language === 'fa' ? 'شهریور' : 'Sep', tempOffset: 6, humOffset: -8, ndviOffset: -6 },
      { name: language === 'fa' ? 'مهر' : 'Oct', tempOffset: -2, humOffset: 12, ndviOffset: 4 }
    ];

    return months.map((m) => {
      // Pseudo-random but stable seed based on coordinates
      const seed = Math.sin(lat * 15.6 + lng * 8.4 + m.tempOffset) * 6;
      const seasonRisk = Math.max(5, Math.min(99, Math.round(baseRisk + m.tempOffset * 1.8 - m.humOffset * 0.7 + seed)));

      let baseNdvi = 60;
      if (region.lvl === 'low') baseNdvi = 80;
      else if (region.lvl === 'medium') baseNdvi = 65;
      else if (region.lvl === 'high') baseNdvi = 48;
      else if (region.lvl === 'critical') baseNdvi = 32;

      const ndvi = Math.max(10, Math.min(95, Math.round(baseNdvi + m.ndviOffset + seed * 0.5)));
      const soilMoisture = Math.max(5, Math.min(95, Math.round((100 - ndvi) * 0.4 + m.humOffset + 25 + seed * 0.8)));

      return {
        month: m.name,
        'ریسک آتش‌سوزی (%)': seasonRisk,
        'Wildfire Risk (%)': seasonRisk,
        'شاخص سلامت گیاهی (NDVI %)': ndvi,
        'Vegetation Health (NDVI %)': ndvi,
        'رطوبت خاک (%)': soilMoisture,
...
<body class="bg-slate-100 text-slate-850 antialiased p-4 md:p-8 min-h-screen">
  ...
  const [leafletLoaded, setLeafletLoaded] = useState<boolean>(!!(window as any).L);
  ...
  useEffect(() => {
    ...
    const loadScripts = async () => {
      try {
        ...
        // Load Leaflet Draw Script if not present
  ...
  }, [leafletLoaded, nodes, mapLayer, shapeMode, circleRadiusKm, customPoints, showRiskZoneOverlay, showStationLocations, showSoilMoisture, showRecentFireHistory, showBiodiversityIndex]);
  ...
  return (
    <div className="space-y-8 animate-fade-in text-slate-100 pb-12 font-sans">
      ...
      <div className="bg-slate-950 p-1 rounded-xl border border-white/5 max-w-2xl mx-auto flex">
        <button
          onClick={() => setActiveSubTab('dashboard')}
          className={`flex-1 py-2.5 text-center font-bold text-[10px] sm:text-xs rounded-lg transition-all ${activeSubTab === 'dashboard' ? 'bg-slate-800 text-rose-400 border border-white/5 shadow-md' : 'text-slate-400 hover:text-white'}`}
        >
        ...
        <button
          ...
          className={`flex-1 py-2.5 text-center font-bold text-[10px] sm:text-xs rounded-lg transition-all ${activeSubTab === 'architecture' ? 'bg-slate-800 text-rose-400 border border-white/5 shadow-md' : 'text-slate-400 hover:text-white'}`}
        >
          <Cpu className="inline-block w-4 h-4 mr-1.5" />
          سخت‌افزار
        </button>
        <button 
          onClick={() => setActiveSubTab('ai-plan')}
          className={`flex-1 py-2.5 text-center font-bold text-[10px] sm:text-xs rounded-lg transition-all border ${activeSubTab === 'ai-plan' ? 'bg-rose-900/30 text-rose-400 border-rose-500/30 shadow-md' : 'text-slate-400 hover:text-white border-transparent'}`}
        >
          <Sparkles className="inline-block w-4 h-4 mr-1.5" />
          {language === 'fa' ? 'طرح پاسخ هوشمند (AI)' : 'AI Strategic Plan'}
        </button>
        <button
          onClick={() => setActiveSubTab('codehub')}
          className={`flex-1 py-2.5 text-center font-bold text-[10px] sm:text-xs rounded-lg transition-all ${activeSubTab === 'codehub' ? 'bg-slate-800 text-rose-400 border border-white/5 shadow-md' : 'text-slate-400 hover:text-white'}`}
        >
        ...
        <button
          ...
          className={`flex-1 py-2.5 text-center font-bold text-[10px] sm:text-xs rounded-lg transition-all ${activeSubTab === 'api' ? 'bg-slate-800 text-rose-400 border border-white/5 shadow-md' : 'text-slate-400 hover:text-white'}`}
        >
      ...
      {/* Tab View 5: AI Strategic Response Plan */}
      {activeSubTab === 'ai-plan' && (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
          <FireResponsePlan 
            location={selectedRegionRisk ? { lat: selectedRegionRisk.center[0], lng: selectedRegionRisk.center[1] } : { lat: activeNode.lat, lng: activeNode.lng }} 
          />
        </div>
      )}
        ...
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              {/* Dynamic Recharts Trend Analysis Panel */}
              <div id="recharts-trend-panel" className="bg-slate-900 border border-white/5 rounded-2xl p-6 shadow-2xl space-y-4">
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="text-lg font-bold text-white">
                        {language === 'fa' 
                          ? 'تحلیلگر پیشرفته روند سلامت بیوم و ریسک حریق تاریخی' 
                          : 'Advanced Biome Health & Historical Wildfire Risk Analytics'}
                      </h3>
                    </div>
                    <p className="text-[11px] text-slate-400">
                      {language === 'fa'
                        ? 'روند فصلی مستخرج از الگوریتم‌های پردازش تصویر لبه و سنسورهای زیرزمینی برای پهنه ژئورفرنس انتخابی.'
                        : 'Seasonal simulation trends derived from image algorithms and telemetry grids for the selected area.'}
                    </p>
                  </div>

                  {selectedRegionRisk && (
                    <div className="bg-slate-950/80 border border-emerald-500/20 px-3 py-1.5 rounded-xl flex items-center gap-2 text-right self-end md:self-auto" dir="rtl">
                      <div className="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-ping" />
                      <span className="text-[10px] text-slate-300 font-medium">
                        {language === 'fa' 
                          ? `داده‌های فعال ناحیه: ${selectedRegionRisk.center[0].toFixed(2)}° N , ${selectedRegionRisk.center[1].toFixed(2)}° E` 
                          : `Active Area: ${selectedRegionRisk.center[0].toFixed(2)}° N , ${selectedRegionRisk.center[1].toFixed(2)}° E`}
                      </span>
                    </div>
                  )}
                </div>
                ...
                <div className="w-full min-h-[320px]">
                  <ResponsiveContainer width="100%" height={320}>
                    <AreaChart
                      data={getHistoricalDataForArea(selectedRegionRisk)}
                    ...
                    >
                      ...
                      <Area 
                        type="monotone" 
                        name={language === 'fa' ? 'امکان وقوع حریق (%)' : 'Wildfire Risk (%)'}
                        dataKey={language === 'fa' ? 'ریسک آتش‌سوزی (%)' : 'Wildfire Risk (%)'} 
                        stroke="#f43f5e" 
                        strokeWidth={2}
                        fillOpacity={1} 
                        fill="url(#colorRisk)" 
                      ...
                      <Area 
                        type="monotone" 
                        name={language === 'fa' ? 'حیات جنگلی (NDVI %)' : 'Vegetation Health (NDVI %)'}
                        dataKey={language === 'fa' ? 'شاخص سلامت گیاهی (NDVI %)' : 'Vegetation Health (NDVI %)'} 
                        stroke="#10b981" 
                        strokeWidth={2}
                        fillOpacity={1} 
                        fill="url(#colorNdvi)" 
                      ...
                      <Line 
                        type="monotone" 
                        name={language === 'fa' ? 'رطوبت خاک (%)' : 'Soil Moisture (%)'}
                        dataKey={language === 'fa' ? 'رطوبت خاک (%)' : 'Soil Moisture (%)'} 
                        stroke="#38bdf8" 
                        strokeWidth={1.5}
                        strokeDasharray="4 4"
                        dot={{ r: 3 }}
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pt-2 text-[10px] text-slate-400">
                  <div className="flex items-center gap-1.5">
                    <span className="w-1.5 h-1.5 rounded-full bg-rose-500" />
                    <span>
                      {language === 'fa' 
                        ? 'فرمول پیش‌بینی: مجهز به ضرایب سرعت باد محلی و نرخ بخارپاشی پوشش زاگرس غربی.' 
                        : 'Predictive algorithm: Adjusted for wind vector modifiers and evapotranspiration indexes.'}
                    </span>
                  </div>
                  {!selectedRegionRisk && (
                    <span className="text-amber-400 bg-amber-500/10 border border-amber-500/20 px-2 py-0.5 rounded">
                      {language === 'fa' 
                        ? 'نمایش میانگین عمومی - برای دیدن تحلیل دقیق پهنه، نقطه‌ای را روی نقشه ترسیم یا انتخاب کنید.' 
                        : 'Showing General Baseline - Select or draw on map for precise localized trends.'}
                    </span>
                  )}
                </div>
              </div>
...
class FireAlertsService {
  ...
  void _displayEmergencyHeadsUpNotification(RemoteMessage message) {
    ...
    _localNotifier.show(
      ...
      <ModuleOptimizer moduleName="SmartFireSense" subModules={['Dashboard', 'Architecture', 'AI-Plan', 'CodeHub']} />
    </div>
  );
};
